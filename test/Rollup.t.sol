...
contract RollupTest is Test {
  ...
  bytes32 constant FIRST_ASSERTION_BLOCKHASH =
    keccak256('FIRST_ASSERTION_BLOCKHASH');
  bytes32 constant FIRST_ASSERTION_SENDROOT =
    keccak256('FIRST_ASSERTION_SENDROOT');
  ...
  function setUp() public {
    ...
    challengeManager = EdgeChallengeManager(
      address(userRollup.challengeManager())
    );
    ...
    address upgradeExecutorExpectedAddress = computeCreateAddress(
  ...
  function testGenesisAssertionConfirmed() external {
    ...
    assertEq(
      userRollup.getAssertion(latestConfirmed).status ==
  ...
  function testConfirmAssertionWhenPaused() public {
    ...
    vm.roll(
      userRollup.getAssertion(genesisHash).firstChildBlock +
  ...
  function testSuccessGetStaker() public {
    ...
    assertEq(
      userRollup.getStakerAddress(userRollup.getStaker(validator1).index),
      validator1
    );
  }
    ...
    )
  {
...

    {
      ...
      randomStates2 = fillStatesInBetween(
        h0,
        h1,
        LAYERZERO_BLOCKEDGE_HEIGHT + 1
      );
    ...
    assertEq(
      userRollup.getAssertion(genesisHash).secondChildBlock,
      block.number
    );
    ...
    returns (SuccessCreateChallengeData memory data)
  {
    ...
    afterState3.endHistoryRoot = keccak256(
      abi.encode(afterState3.endHistoryRoot)
    );
      ...
      CreateEdgeArgs({
        ...
        prefixProof: abi.encode(
          ...
          ProofUtils.generatePrefixProof(
            1,
            ArrayUtilsLib.slice(randomStates1, 1, randomStates1.length)
          )
        ),
        proof: abi.encode(
          ProofUtils.generateInclusionProof(
            ProofUtils.rehashed(randomStates1),
            randomStates1.length - 1
          ),
    ...
    bytes32 e2Id = challengeManager.createLayerZeroEdge(
      CreateEdgeArgs({
        ...
        prefixProof: abi.encode(
          ...
          ProofUtils.generatePrefixProof(
            1,
            ArrayUtilsLib.slice(randomStates2, 1, randomStates2.length)
          )
        ),
        proof: abi.encode(
          ProofUtils.generateInclusionProof(
            ProofUtils.rehashed(randomStates2),
            randomStates2.length - 1
          ),
          AssertionStateData(data.beforeState, bytes32(0), bytes32(0)),
          AssertionStateData(
            data.afterState2,
  ...
  function testSuccessUpgradePrimary() public {
    address ori_secondary_impl = address(
      uint160(
        uint256(vm.load(address(userRollup), _IMPLEMENTATION_SECONDARY_SLOT))
      )
    );
    ...
    address new_primary_impl = address(
      uint160(
        uint256(vm.load(address(userRollup), _IMPLEMENTATION_PRIMARY_SLOT))
      )
    );
  ...
  function testAssertionHash() public {
    ...
    bytes32 expectedHash = keccak256(
      abi.encodePacked(parentHash, astate.hash(), inboxAcc)
    );
    assertEq(
      RollupLib.assertionHash(parentHash, astate, inboxAcc),
      expectedHash,
      'Unexpected hash'
    );
  }
  ...
  function testBaseStake() public {
    ...
    assertEq(
      adminRollup.baseStake(),
      BASE_STAKE + 1,
      'Invalid after increase base stake'
    );
