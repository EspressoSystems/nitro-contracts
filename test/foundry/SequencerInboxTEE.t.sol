...
contract SequencerInboxTest is Test {
  ...
  address nitroEnclaveVerifierAddr =
  ...
  function _computeEIP712Digest(
    bytes32 reportDataHash
  ) internal view returns (bytes32) {
    bytes32 structHash = keccak256(
      abi.encode(ESPRESSO_TEE_VERIFIER_TYPE_HASH, reportDataHash)
    );
  ...
  function setUp() public {
    ...
    // Ensure NitroEnclaveVerifier has code on the fork (may not exist at all fork heights)
    ...
    espressoNitroTEEVerifier = new EspressoNitroTEEVerifier(
      address(espressoTEEVerifier),
      nitroEnclaveVerifierAddr
    );

    // Wire the real nitro verifier into the TEE verifier and register enclave hash
    ...
    // Mock the Nitro signer as valid (avoids needing real ZK proof registration data)
    ...
    // --- Rollup / Bridge / SequencerInbox setup (unchanged) ---
  ...
  function testAddSequencerL2BatchFromOriginNitro() public {
    ...
    // Sign with EIP-712 typed data (new verify() format)
