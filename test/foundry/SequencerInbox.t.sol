...
contract SequencerInboxTest is Test {
  ...
  bytes32 mrSigner =
    bytes32(0x0c8242bba090f54b10de0c2d1ca4b633b9c08b7178451c71d737c214b72fc836);
  ...
  function deployRollup(
    ...
    SequencerInbox seqInbox = SequencerInbox(
      address(
        new TransparentUpgradeableProxy(address(seqInboxImpl), proxyAdmin, '')
      )
    );
    seqInbox.initialize(
      bridge,
  ...
  function deployFeeTokenBasedRollup()
    internal
    returns (SequencerInbox, ERC20Bridge)
  {
    RollupMock rollupMock = new RollupMock(rollupOwner);
    ...
    address nativeToken = address(
      new ERC20PresetMinterPauser('Appchain Token', 'App')
    );
  ...
  function expectEvents(
    ...
    bytes32 afterAcc = keccak256(
      abi.encodePacked(beforeAcc, dataHash, delayedAcc)
    );
  ...
  function testConstructor() public {
    SequencerInbox seqInboxLogic = new SequencerInbox(
      MAX_DATA_SIZE,
      dummyReader4844,
      false,
      false
    ...
    assertEq(
      seqInboxLogic.maxDataSize(),
      MAX_DATA_SIZE,
      'Invalid MAX_DATA_SIZE'
    );
    assertEq(seqInboxLogic.isUsingFeeToken(), false, 'Invalid isUsingFeeToken');

    SequencerInbox seqInboxProxy = SequencerInbox(
      TestUtil.deployProxy(address(seqInboxLogic))
    );
    assertEq(
      seqInboxProxy.maxDataSize(),
      MAX_DATA_SIZE,
      'Invalid MAX_DATA_SIZE'
    );
    ...
    assertEq(
      seqInboxLogicFeeToken.maxDataSize(),
      MAX_DATA_SIZE,
      'Invalid MAX_DATA_SIZE'
    );
    assertEq(
      seqInboxLogicFeeToken.isUsingFeeToken(),
      true,
      'Invalid isUsingFeeToken'
    );
    ...
    assertEq(
      seqInboxProxyFeeToken.maxDataSize(),
      MAX_DATA_SIZE,
      'Invalid MAX_DATA_SIZE'
    );
    assertEq(
      seqInboxProxyFeeToken.isUsingFeeToken(),
      true,
      'Invalid isUsingFeeToken'
    );
  }
  ...
  function testInitialize(BufferConfig memory bufferConfig) public {
    Bridge _bridge = Bridge(
      address(
        new TransparentUpgradeableProxy(address(new Bridge()), proxyAdmin, '')
      )
    );
    ...
    SequencerInbox seqInboxProxy = SequencerInbox(
      TestUtil.deployProxy(seqInboxLogic)
    );
    ...
    assertEq(seqInboxProxy.isUsingFeeToken(), false, 'Invalid isUsingFeeToken');
    assertEq(
      address(seqInboxProxy.bridge()),
      address(_bridge),
      'Invalid bridge'
    );
    assertEq(
      address(seqInboxProxy.rollup()),
      address(_bridge.rollup()),
      'Invalid rollup'
    );
  }
  ...
  function testInitialize_FeeTokenBased(
    ...
    _bridge.initialize(
      IOwnable(address(new RollupMock(rollupOwner))),
      nativeToken
    );
  ...
  function testAddSequencerL2BatchFromOrigin_ArbitrumHostedFeeTokenBased()
    public
  {
  ...
  function testAddSequencerL2BatchFromOriginReverts() public {
    ...
    bytes memory authenticatedData = bytes.concat(
      seqInbox.DATA_BLOB_HEADER_FLAG(),
      data
    );
    vm.expectRevert(
      abi.encodeWithSelector(InvalidHeaderFlag.selector, authenticatedData[0])
    );
    vm.prank(tx.origin);
    seqInbox.addSequencerL2BatchFromOrigin(
      sequenceNumber,
  ...
  function testPostUpgradeInitAlreadyInitBuffer(
    ...
    (SequencerInbox seqInbox, ) = deployRollup(
      false,
      false,
      bufferConfigDefault
    );
    ...
    TransparentUpgradeableProxy(payable(address(seqInbox))).upgradeToAndCall(
      ...
      abi.encodeWithSelector(
        SequencerInbox.postUpgradeInit.selector,
        bufferConfig
      )
    );
  ...
  function testSetMaxTimeVariation(
    ...
    if (
      delayBlocks > uint256(type(uint64).max) ||
      futureBlocks > uint256(type(uint64).max) ||
  ...
  function test_updateRollupAddress_revert_NotOwner() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(NotOwner.selector, address(this), address(1337))
    );
    seqInbox.updateRollupAddress();
  }

  function test_postUpgradeInit_revert_NotDelayBufferable() public {
    (SequencerInbox seqInbox, , address seqInboxImpl) = deployRollup(
      false,
      false,
      bufferConfigDefault
    );
