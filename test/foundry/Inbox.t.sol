...
contract InboxTest is AbsInboxTest {
  ...
  function test_createRetryableTicket_FromContract() public {
    ...
    // expect event
    uint256 uintAlias = uint256(
      uint160(AddressAliasHelper.applyL1ToL2Alias(sender))
    );
    vm.expectEmit(true, true, true, true);
    emit InboxMessageDelivered(
      0,
  ...
  function test_createRetryableTicket_revert_InsufficientSubmissionCost()
    public
  {
  ...
  function test_unsafeCreateRetryableTicket_NotRevertingOnInsufficientValue()
    public
  {
    uint256 bridgeEthBalanceBefore = address(bridge).balance;
    uint256 userEthBalanceBefore = address(user).balance;
