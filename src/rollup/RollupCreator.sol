...
contract RollupCreator is Ownable {
  ...
  function createRollup(
  ...
  ) public payable returns (address) {
    {
      ...
      require(
        deployParams.maxDataSize == ethInbox.maxDataSize(),
    ...
    RollupProxy rollup = new RollupProxy{
      salt: keccak256(abi.encode(deployParams))
    }();
    BridgeCreator.BridgeContracts memory bridgeContracts = bridgeCreator
      .createBridge(
        address(proxyAdmin),
        address(rollup),
    ...
    for (uint256 i = 0; i < deployParams.batchPosters.length; i++) {
      bridgeContracts.sequencerInbox.setIsBatchPoster(
        deployParams.batchPosters[i],
        true
      );
    }
    if (deployParams.batchPosterManager != address(0)) {
      bridgeContracts.sequencerInbox.setBatchPosterManager(
  ...
  function _deployFactories(
    ...
    if (_nativeToken == address(0)) {
      ...
      // do it
      l2FactoriesDeployer.perform{ value: cost }(
        _inbox,
        _nativeToken,
        _maxFeePerGas
      );
